...
void Motors::routine() {
	...
	if (RSQR) {
		...
		*/
		...
		bool in_plane = (location() == 1) ? true : false;
		if (in_plane) {
			epsilon_ang = 2 * atan(Dy / (Dx + Ddist)) - theta_act;
		}
		else {
			float Dx_ortho = -(director[0][1] - director[2][1]);
			float Dy_ortho = director[0][0] - director[2][0];
			float Ddist_ortho = sqrt(Dx_ortho*Dx_ortho + Dy_ortho*Dy_ortho);
			epsilon_ang = 2* atan(Dy_ortho / (Dx_ortho + Ddist_ortho)) - atan(Dy / (Dx + Ddist));
		}
	}
	else if (ROT) {
